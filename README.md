...
ZettelVault reads every note from one or more source vaults, classifies each into the [PARA](https://fortelabs.com/blog/para/) method (Projects / Areas / Resources / Archive), then decomposes each note into atomic [Zettelkasten](https://en.wikipedia.org/wiki/Zettelkasten) notes -- one idea per note, heavily cross-linked. It uses [DSPy](https://github.com/stanfordnlp/dspy) for structured LLM interaction, with [dspy.RLM](https://dspy.ai/api/modules/RLM) (Recursive Language Models) as the primary decomposition strategy.

This project also serves as **reference code for using dspy.RLM for document decomposition**, a technique applicable to any use case where long-form documents need to be split into structured, atomic units.
...
Reads all `.md` files from the source vault(s) using [`vlt`](https://github.com/RamXX/vlt) (an Obsidian vault CLI for AI agents). Multiple source vaults are supported -- pass space-separated vault names and ZettelVault merges them (first vault wins on title collision). No Obsidian desktop app required for processing, but recommended for viewing results.

### Step 2: Classify (dspy.Predict)

Each note is classified into:
...
Uses a typed DSPy `Signature` with `Literal` type for PARA buckets, ensuring valid output.
...
Each classified note is decomposed into atomic Zettelkasten notes. This is the core of the pipeline and uses a **three-level fallback strategy**:
...
Before decomposition, a **concept index** maps meaningful words to note titles, enabling cross-link suggestions. Each note receives a list of the most conceptually similar note titles as context.

Output format uses `===`-delimited markdown blocks (not JSON -- see [Design Decisions](#why-markdown-over-json)).
...
Decomposition results are checkpointed to `atomic_notes.json` after every note. If the cache exists on a subsequent run, already-decomposed notes (matched by source title) are skipped automatically (progressive processing).

### Step 4: Write

Atomic notes are written to the destination vault with:
...
A Map of Content (MOC) note per domain is generated in `MOC/`, containing `[[wikilinks]]` to all atomic notes in that domain.
...
After writing, ZettelVault scans the destination vault and resolves orphan `[[wikilinks]]` -- links that point to notes that do not exist:
...
This ensures the output vault has a clean, navigable link graph.
...
Tested on 4 source notes from an Obsidian vault, using `qwen/qwen3.5-35b-a3b` via OpenRouter (Parasail provider):
...
- **RLM solved the deterministic failure case.** A bullet-heavy note always failed with Predict across all temperatures (0.1--0.7) and retry counts. RLM decomposed it successfully on the first attempt by writing code to iterate over bullet points.
...
We tested multiple open-source models as RLM orchestrators across two dimensions: **quantitative metrics** (cost, speed, convergence) and **qualitative assessment** (output quality, content fidelity, link richness). All tests used the same 4 source notes, run via [OpenRouter](https://openrouter.ai).

These results reflect only the models we tested. Others have reported success with `openai/gpt-oss-120b` and `glm-5`, and closed-source SOTA models would likely perform even better. Our focus is on open-source models because many users want to run this locally via [LM Studio](https://lmstudio.ai/), [Ollama](https://ollama.com/), or similar tools.
...
We manually reviewed every atom produced by each model, comparing against the source notes for content fidelity, duplication, granularity, and cross-linking quality.
...
The fallback chain (RLM -> Predict with retry -> passthrough) ensures the pipeline never fails:
...
This means the pipeline can process an entire vault without manual intervention.
...
- [vlt](https://github.com/RamXX/vlt) (Obsidian vault CLI)
- An [OpenRouter](https://openrouter.ai) API key (or a compatible API endpoint)
...
Local overrides merge deeply -- you only need to specify the keys you want to change:
...
RLM decomposes documents in two distinct roles:
...
```bash
# Full run
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest

# Disable RLM (use Predict only)
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest --no-rlm

# Process only first 10 notes
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest --limit 10

# Dry run (no writes)
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest --dry-run

# Skip classification (load from cache)
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest --skip-classification

# Skip decomposition (load from cache)
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest --skip-decomposition

# Multiple source vaults
uv run --env-file .env -- python zettelvault_dspy.py VaultA VaultB ~/path/to/dest

# Custom config
uv run --env-file .env -- python zettelvault_dspy.py MyVault ~/path/to/dest --config config.local.yaml
```
...
```
zettelvault/
  zettelvault_dspy.py     # Main pipeline (read, classify, decompose, write, resolve links)
  pricing.py              # Cost tracking module (OpenRouter API + DSPy history)
  config.yaml             # Default configuration (models, RLM, pipeline)
...
- [Kimi K2.5](https://huggingface.co/moonshotai/Kimi-K2.5) -- Moonshot AI's multimodal agentic model (best quality in our tests)
- [Qwen3.5-35B-A3B](https://huggingface.co/Qwen/Qwen3.5-35B-A3B) -- 35B MoE model (3B active), 262K context
- [vlt](https://github.com/RamXX/vlt) -- Obsidian vault CLI for AI agents
